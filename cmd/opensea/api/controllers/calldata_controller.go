...
import (
	...
	"gitlab.com/meta-node/meta-node/cmd/opensea/network"
...
func (caller *CallData)TryCall(ctx *gin.Context) {
	transferMu.Lock()
...
func (caller *CallData)EncodeAbi(call map[string]interface{}) []byte {
	...
	// 	panic(err)
	// }
	reader, err := os.Open("./abi/marketplace.json")
	if err != nil {
		log.Fatalf("Error occured while reading %s", "./abi/marketplace.json")
	}
