...
type (
	Node interface {
		...
		Flag() bool
		Deserialize([]byte, storage.Storage) (err error)
		Copy() Node
	}
...
func FetchNodeFromStorage(nodeHash []byte, storage storage.Storage) (Node, error) { //nodeHash là hash của node
...
func (n *FullNode) Deserialize(bFN []byte, storage storage.Storage) (err error) {
	protoFN := &pb.MPTFullNode{}
...
func (n *FullNode) Save(s storage.Storage) {
	s.Put(n.Hash(), n.Serialize())
}

func (n *FullNode) Copy() Node { copy := *n; return &copy }
...
func (n *ShortNode) Deserialize(bSN []byte, storage storage.Storage) (err error) {
	protoSN := &pb.MPTShortNode{}
...
func (n *ShortNode) Save(s storage.Storage) {
	s.Put(n.Hash(), n.Serialize())

}

func (n *ShortNode) Copy() Node { copy := *n; return &copy }
...
func (n *ValueNode) Deserialize(value []byte, storage storage.Storage) (err error) {
	n.Value = value
	return nil
}
func (n *ValueNode) Save(s storage.Storage) {
	s.Put(n.Hash(), n.Serialize())
}
...
func (n *ValueNode) Copy() Node { copy := *n; return &copy }
